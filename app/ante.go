...
func newAnteHandler(options ante.HandlerOptions, ibcKeeper *ibckeeper.Keeper,WasmConfig  wasmTypes.WasmConfig, TxCounterStoreKey sdk.StoreKey, cdc codec.BinaryCodec) (*sdk.AnteHandler, error) {
	...
	wasmGasDecorator:= wasmKeeper.NewLimitSimulationGasDecorator(WasmConfig.SimulationGasLimit)
	wasmCounTxDecorator := wasmKeeper.NewCountTXDecorator(TxCounterStoreKey)

	addlDecorators := []sdk.AnteDecorator{ibcAnteDecorator, minCommissionDecorator,wasmGasDecorator,wasmCounTxDecorator}
