...
func (s EVMAnteTestSuite) TestMinGasPriceDecorator() {
	...
	for _, et := range execTypes {
		for _, tc := range testCases {
			s.Run(et.name+"_"+tc.name, func() {
				...
				dec := evmante.NewMinGasPriceDecorator(s.app.FeeMarketKeeper, s.app.EVMKeeper)
...
func (s EVMAnteTestSuite) TestEthMinGasPriceDecorator() {
	denom := evmtypes.DefaultEVMDenom
	...
	for _, et := range execTypes {
		for _, tc := range testCases {
			s.Run(et.name+"_"+tc.name, func() {
				// s.SetupTest(et.isCheckTx)
				s.SetupTest()
				dec := evmante.NewEthMinGasPriceDecorator(s.app.FeeMarketKeeper, s.app.EVMKeeper)
