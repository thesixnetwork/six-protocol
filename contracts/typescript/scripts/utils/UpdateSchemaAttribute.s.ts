...
import upateAttribute from "../../../resources/UpdateAttribute/update_attribute.json";
...
async function main() {
  ...
  const encodeBase64UpdagteAttribute = Buffer.from(JSON.stringify(upateAttribute)).toString(
    "base64"
  );
  ...
  // @ts-ignore
  const tx = await nftmngr.connect(wallet).updateSchemaAttribute(schemaCode,encodeBase64UpdagteAttribute)
