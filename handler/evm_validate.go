...
func ValidateEVM(message string, signature string, eth_address string) (pass bool,err error) {
	...
	if verified := crypto.VerifySignature(sigPublicKey, hash.Bytes(), signatureNoRecoverID); !verified {
		return false, sdkerrors.Wrap(sdkerrors.ErrInvalidRequest, "verify failed")
	}
