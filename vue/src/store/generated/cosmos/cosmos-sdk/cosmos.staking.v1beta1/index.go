...
import { ValidatorApproval } from "./module/types/cosmos/staking/v1beta1/staking"
import { WhitelistDelegator } from "./module/types/cosmos/staking/v1beta1/whitelist"


export { StakeAuthorization, StakeAuthorization_Validators, LastValidatorPower, HistoricalInfo, CommissionRates, Commission, Description, Validator, ValAddresses, DVPair, DVPairs, DVVTriplet, DVVTriplets, Delegation, UnbondingDelegation, UnbondingDelegationEntry, RedelegationEntry, Redelegation, Params, DelegationResponse, RedelegationEntryResponse, RedelegationResponse, Pool, ValidatorApproval, WhitelistDelegator };
...
const getDefaultState = () => {
	return {
				...
				_Structure: {
						...
						ValidatorApproval: getStructure(ValidatorApproval.fromPartial({})),
						WhitelistDelegator: getStructure(WhitelistDelegator.fromPartial({})),
...
export default {
	...
	getters: {
				getValidatorApproval: (state) => (params = { params: {}}) => {
					if (!(<any> params).query) {
						(<any> params).query=null
					}
			return state.ValidatorApproval[JSON.stringify(params)] ?? {}
		},
			...
			return state.Params[JSON.stringify(params)] ?? {}
				...
				getWhitelistdelegator: (state) => (params = { params: {}}) => {
					if (!(<any> params).query) {
						(<any> params).query=null
					}
			return state.Whitelistdelegator[JSON.stringify(params)] ?? {}
		},
				getWhitelistdelegatorAll: (state) => (params = { params: {}}) => {
					if (!(<any> params).query) {
						(<any> params).query=null
					}
			return state.WhitelistdelegatorAll[JSON.stringify(params)] ?? {}
		},
	...
	actions: {
		...
		async QueryValidatorApproval({ commit, rootGetters, getters }, { options: { subscribe, all} = { subscribe:false, all:false}, params, query=null }) {
			try {
				const key = params ?? {};
				const queryClient=await initQueryClient(rootGetters)
				let value= (await queryClient.queryValidatorApproval()).data
				
					
				commit('QUERY', { query: 'ValidatorApproval', key: { params: {...key}, query}, value })
				if (subscribe) commit('SUBSCRIBE', { action: 'QueryValidatorApproval', payload: { options: { all }, params: {...key},query }})
				return getters['getValidatorApproval']( { params: {...key}, query}) ?? {}
			} catch (e) {
				throw new Error('QueryClient:QueryValidatorApproval API Node Unavailable. Could not perform query: ' + e.message)
		...
		async QueryWhitelistdelegator({ commit, rootGetters, getters }, { options: { subscribe, all} = { subscribe:false, all:false}, params, query=null }) {
			try {
				const key = params ?? {};
				const queryClient=await initQueryClient(rootGetters)
				let value= (await queryClient.queryWhitelistdelegator( key.validator)).data
				
					
				commit('QUERY', { query: 'Whitelistdelegator', key: { params: {...key}, query}, value })
				if (subscribe) commit('SUBSCRIBE', { action: 'QueryWhitelistdelegator', payload: { options: { all }, params: {...key},query }})
				return getters['getWhitelistdelegator']( { params: {...key}, query}) ?? {}
			} catch (e) {
				throw new Error('QueryClient:QueryWhitelistdelegator API Node Unavailable. Could not perform query: ' + e.message)
		...
		async QueryWhitelistdelegatorAll({ commit, rootGetters, getters }, { options: { subscribe, all} = { subscribe:false, all:false}, params, query=null }) {
			try {
				const key = params ?? {};
				const queryClient=await initQueryClient(rootGetters)
				let value= (await queryClient.queryWhitelistdelegatorAll(query)).data
				
					
				while (all && (<any> value).pagination && (<any> value).pagination.next_key!=null) {
					let next_values=(await queryClient.queryWhitelistdelegatorAll({...query, 'pagination.key':(<any> value).pagination.next_key})).data
					value = mergeResults(value, next_values);
				}
				commit('QUERY', { query: 'WhitelistdelegatorAll', key: { params: {...key}, query}, value })
				if (subscribe) commit('SUBSCRIBE', { action: 'QueryWhitelistdelegatorAll', payload: { options: { all }, params: {...key},query }})
				return getters['getWhitelistdelegatorAll']( { params: {...key}, query}) ?? {}
			} catch (e) {
				throw new Error('QueryClient:QueryWhitelistdelegatorAll API Node Unavailable. Could not perform query: ' + e.message)
	...
	gas: "200000" }, memo})
		...
		async sendMsgCreateWhitelistDelegator({ rootGetters }, { value, fee = [], memo = '' }) {
			try {
				const txClient=await initTxClient(rootGetters)
				const msg = await txClient.msgCreateWhitelistDelegator(value)
				const result = await txClient.signAndBroadcast([msg], {fee: { amount: fee, 
	gas: "200000" }, memo})
				return result
			} catch (e) {
				if (e == MissingWalletError) {
					throw new Error('TxClient:MsgCreateWhitelistDelegator:Init Could not initialize signing client. Wallet is required.')
				}else{
					throw new Error('TxClient:MsgCreateWhitelistDelegator:Send Could not broadcast Tx: '+ e.message)
				}
			}
		},
	...
	gas: "200000" }, memo})
		...
		async sendMsgSetValidatorApproval({ rootGetters }, { value, fee = [], memo = '' }) {
			try {
				const txClient=await initTxClient(rootGetters)
				const msg = await txClient.msgSetValidatorApproval(value)
				const result = await txClient.signAndBroadcast([msg], {fee: { amount: fee, 
	gas: "200000" }, memo})
				return result
			} catch (e) {
				if (e == MissingWalletError) {
					throw new Error('TxClient:MsgSetValidatorApproval:Init Could not initialize signing client. Wallet is required.')
				}else{
					throw new Error('TxClient:MsgSetValidatorApproval:Send Could not broadcast Tx: '+ e.message)
				}
			}
		},
		async sendMsgDeleteWhitelistDelegator({ rootGetters }, { value, fee = [], memo = '' }) {
			try {
				const txClient=await initTxClient(rootGetters)
				const msg = await txClient.msgDeleteWhitelistDelegator(value)
				const result = await txClient.signAndBroadcast([msg], {fee: { amount: fee, 
	gas: "200000" }, memo})
				return result
			} catch (e) {
				if (e == MissingWalletError) {
					throw new Error('TxClient:MsgDeleteWhitelistDelegator:Init Could not initialize signing client. Wallet is required.')
				}else{
					throw new Error('TxClient:MsgDeleteWhitelistDelegator:Send Could not broadcast Tx: '+ e.message)
				}
			}
		},
		...
		async MsgCreateWhitelistDelegator({ rootGetters }, { value }) {
			try {
				const txClient=await initTxClient(rootGetters)
				const msg = await txClient.msgCreateWhitelistDelegator(value)
				return msg
				...
				if (e == MissingWalletError) {
					...
					throw new Error('TxClient:MsgCreateWhitelistDelegator:Create Could not create message: ' + e.message)
				}
			}
		},
		...
		async MsgSetValidatorApproval({ rootGetters }, { value }) {
			try {
				const txClient=await initTxClient(rootGetters)
				const msg = await txClient.msgSetValidatorApproval(value)
				return msg
				...
				if (e == MissingWalletError) {
					...
					throw new Error('TxClient:MsgSetValidatorApproval:Create Could not create message: ' + e.message)
				}
			}
		},
		async MsgDeleteWhitelistDelegator({ rootGetters }, { value }) {
			try {
				const txClient=await initTxClient(rootGetters)
				const msg = await txClient.msgDeleteWhitelistDelegator(value)
				return msg
				...
				if (e == MissingWalletError) {
					...
					throw new Error('TxClient:MsgDeleteWhitelistDelegator:Create Could not create message: ' + e.message)
				}
			}
		},
