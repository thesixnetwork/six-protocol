...
import { Params } from "./module/types/ethermint/feemarket/v2/feemarket"
...
export default {
	...
	actions: {
		init({ dispatch, rootGetters }) {
			console.log('Vuex module: ethermint.feemarket.v2 initialized!')
		...
		async QueryBlockGas({ commit, rootGetters, getters }, { options: { subscribe, all} = { subscribe:false, all:false}, params, query=null }) {
			try {
				const key = params ?? {};
				const queryClient=await initQueryClient(rootGetters)
				let value= (await queryClient.queryBlockGas()).data
