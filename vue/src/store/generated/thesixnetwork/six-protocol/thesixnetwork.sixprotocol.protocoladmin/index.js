    ...
    actions: {
        ...
        async QueryGroup({ commit, rootGetters, getters }, { options: { subscribe, all } = { subscribe: false, all: false }, params, query = null }) {
            try {
                ...
                if (subscribe)
                    commit('SUBSCRIBE', { action: 'QueryGroup', payload: { options: { all }, params: { ...key }, query } });
                return getters['getGroup']({ params: { ...key }, query }) ?? {};
            }
            catch (e) {
                throw new SpVuexError('QueryClient:QueryGroup', 'API Node Unavailable. Could not perform query: ' + e.message);
            }
        },
        async QueryGroupAll({ commit, rootGetters, getters }, { options: { subscribe, all } = { subscribe: false, all: false }, params, query = null }) {
            try {
                ...
                while (all && value.pagination && value.pagination.next_key != null) {
                    let next_values = (await queryClient.queryGroupAll({ ...query, 'pagination.key': value.pagination.next_key })).data;
                    value = mergeResults(value, next_values);
                }
                commit('QUERY', { query: 'GroupAll', key: { params: { ...key }, query }, value });
                if (subscribe)
                    commit('SUBSCRIBE', { action: 'QueryGroupAll', payload: { options: { all }, params: { ...key }, query } });
                return getters['getGroupAll']({ params: { ...key }, query }) ?? {};
            }
            catch (e) {
                throw new SpVuexError('QueryClient:QueryGroupAll', 'API Node Unavailable. Could not perform query: ' + e.message);
            }
        },
        ...
        async QueryAdminAll({ commit, rootGetters, getters }, { options: { subscribe, all } = { subscribe: false, all: false }, params, query = null }) {
            try {
                ...
                while (all && value.pagination && value.pagination.next_key != null) {
                    let next_values = (await queryClient.queryAdminAll({ ...query, 'pagination.key': value.pagination.next_key })).data;
                    value = mergeResults(value, next_values);
                }
        ...
        async QueryListAdminOfGroup({ commit, rootGetters, getters }, { options: { subscribe, all } = { subscribe: false, all: false }, params, query = null }) {
            try {
                ...
                while (all && value.pagination && value.pagination.next_key != null) {
                    let next_values = (await queryClient.queryListAdminOfGroup(key.group, { ...query, 'pagination.key': value.pagination.next_key })).data;
                    value = mergeResults(value, next_values);
                }
                commit('QUERY', { query: 'ListAdminOfGroup', key: { params: { ...key }, query }, value });
                if (subscribe)
                    commit('SUBSCRIBE', { action: 'QueryListAdminOfGroup', payload: { options: { all }, params: { ...key }, query } });
        ...
        async sendMsgCreateGroup({ rootGetters }, { value, fee = [], memo = '' }) {
            try {
                ...
                if (e == MissingWalletError) {
                    throw new SpVuexError('TxClient:MsgCreateGroup:Init', 'Could not initialize signing client. Wallet is required.');
                }
                else {
