...
func networkWithNFTFeeConfigObjects(t *testing.T) (*network.Network, types.NFTFeeConfig) {
	t.Helper()
...
func TestShowNFTFeeConfig(t *testing.T) {
	net, obj := networkWithNFTFeeConfigObjects(t)
	...
	for _, tc := range []struct {
		...
		t.Run(tc.desc, func(t *testing.T) {
			...
			out, err := clitestutil.ExecTestCLICmd(ctx, cli.CmdShowNFTFeeConfig(), args)
			if tc.err != nil {
				...
				require.NoError(t, err)
				var resp types.QueryGetNFTFeeConfigResponse
				require.NoError(t, net.Config.Codec.UnmarshalJSON(out.Bytes(), &resp))
				require.NotNil(t, resp.NFTFeeConfig)
				require.Equal(t,
					nullify.Fill(&tc.obj),
