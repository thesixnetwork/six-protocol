...
func CmdVoteDisableVirtualSchema() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "vote-disable-virtual-schema [virtual-nft-schema-code] [your-nft-schema-code] [yes(y)  no(n)]",
		Short: "Broadcast message voteDisableVirtualSchema",
		Args:  cobra.ExactArgs(3),
		RunE: func(cmd *cobra.Command, args []string) (err error) {
			proposalId := args[0]
			argNftSchemaName := args[1]
			...
			msg := types.NewMsgVoteDisableVirtualSchema(
				clientCtx.GetFromAddress().String(),
				proposalId,
				argNftSchemaName,
				selectedVote,
...
func CmdVoteEnableVirtualSchema() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "vote-enable-virtual-schema [virtual-nft-schema-code] [your-nft-schema-code] [yes(y)  no(n)]",
		Short: "Broadcast message voteEnableVirtualSchema",
		Args:  cobra.ExactArgs(3),
		RunE: func(cmd *cobra.Command, args []string) (err error) {
			proposalId := args[0]
			argNftSchemaName := args[1]
			...
			msg := types.NewMsgVoteEnableVirtualSchema(
				clientCtx.GetFromAddress().String(),
				proposalId,
				argNftSchemaName,
				selectedVote,
