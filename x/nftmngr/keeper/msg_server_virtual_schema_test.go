...
func TestVirtualSchemaMsgServerUpdate(t *testing.T) {
	creator := "A"

	for _, tc := range []struct {
		desc    string
		request *types.MsgUpdateVirtualSchema
		err     error
	}{
		{
			desc: "Completed",
			request: &types.MsgUpdateVirtualSchema{Creator: creator,
				Index: strconv.Itoa(0),
			},
		},
		{
			desc: "Unauthorized",
			request: &types.MsgUpdateVirtualSchema{Creator: "B",
				Index: strconv.Itoa(0),
			},
		...
		t.Run(tc.desc, func(t *testing.T) {
			...
			_, err = srv.UpdateVirtualSchema(wctx, tc.request)
			if tc.err != nil {
