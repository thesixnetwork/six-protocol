...
func TestActionByRefIdQuerySingle(t *testing.T) {
	...
	msgs := createNActionByRefId(k, ctx, 2)
	tests := []struct {
		desc     string
		...
		{
			desc:     "First",
			request:  &types.QueryGetActionByRefIdRequest{RefId: msgs[0].RefId},
			response: &types.QueryGetActionByRefIdResponse{ActionByRefId: msgs[0]},
		},
		{
			desc:     "Second",
			request:  &types.QueryGetActionByRefIdRequest{RefId: msgs[1].RefId},
			response: &types.QueryGetActionByRefIdResponse{ActionByRefId: msgs[1]},
		},
		{
			desc:    "KeyNotFound",
			request: &types.QueryGetActionByRefIdRequest{RefId: strconv.Itoa(100000)},
	...
	for _, tc := range tests {
		t.Run(tc.desc, func(t *testing.T) {
			response, err := k.ActionByRefId(ctx, tc.request)
			if tc.err != nil {
...
func TestActionByRefIdQueryPaginated(t *testing.T) {
	k, ctx := keepertest.NftmngrKeeper(t)
	msgs := createNActionByRefId(k, ctx, 5)
	...
	t.Run("ByOffset", func(t *testing.T) {
		...
		for i := 0; i < len(msgs); i += step {
			resp, err := k.ActionByRefIdAll(ctx, request(nil, uint64(i), uint64(step), false))
			require.NoError(t, err)
	...
	t.Run("ByKey", func(t *testing.T) {
		...
		var next []byte
		for i := 0; i < len(msgs); i += step {
			resp, err := k.ActionByRefIdAll(ctx, request(next, 0, uint64(step), false))
			require.NoError(t, err)
			require.LessOrEqual(t, len(resp.ActionByRefId), step)
			require.Subset(t,
				nullify.Fill(msgs),
	...
	t.Run("Total", func(t *testing.T) {
		resp, err := k.ActionByRefIdAll(ctx, request(nil, 0, 0, true))
		require.NoError(t, err)
	...
	t.Run("InvalidRequest", func(t *testing.T) {
		_, err := k.ActionByRefIdAll(ctx, nil)
