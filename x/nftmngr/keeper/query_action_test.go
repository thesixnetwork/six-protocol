...
func TestActionOfSchemaQuerySingle(t *testing.T) {
	...
	msgs := createNActionOfSchema(k, ctx, 2)
	tests := []struct {
		desc     string
		...
		{
			desc:     "First",
			request:  &types.QueryGetActionOfSchemaRequest{NftSchemaCode: msgs[0].NftSchemaCode, Name: msgs[0].Name},
			response: &types.QueryGetActionOfSchemaResponse{ActionOfSchema: msgs[0]},
		},
		{
			desc:     "Second",
			request:  &types.QueryGetActionOfSchemaRequest{NftSchemaCode: msgs[1].NftSchemaCode, Name: msgs[1].Name},
			response: &types.QueryGetActionOfSchemaResponse{ActionOfSchema: msgs[1]},
		},
		{
			desc:    "KeyNotFound",
			request: &types.QueryGetActionOfSchemaRequest{NftSchemaCode: strconv.Itoa(100000), Name: strconv.Itoa(100000)},
	...
	for _, tc := range tests {
		t.Run(tc.desc, func(t *testing.T) {
			response, err := k.ActionOfSchema(ctx, tc.request)
			if tc.err != nil {
...
func TestActionOfSchemaQueryPaginated(t *testing.T) {
	k, ctx := keepertest.NftmngrKeeper(t)
	msgs := createNActionOfSchema(k, ctx, 5)
	...
	t.Run("ByOffset", func(t *testing.T) {
		...
		for i := 0; i < len(msgs); i += step {
			resp, err := k.ActionOfSchemaAll(ctx, request(nil, uint64(i), uint64(step), false))
			require.NoError(t, err)
	...
	t.Run("ByKey", func(t *testing.T) {
		...
		var next []byte
		for i := 0; i < len(msgs); i += step {
			resp, err := k.ActionOfSchemaAll(ctx, request(next, 0, uint64(step), false))
			require.NoError(t, err)
			require.LessOrEqual(t, len(resp.ActionOfSchema), step)
			require.Subset(t,
				nullify.Fill(msgs),
	...
	t.Run("Total", func(t *testing.T) {
		resp, err := k.ActionOfSchemaAll(ctx, request(nil, 0, 0, true))
		require.NoError(t, err)
	...
	t.Run("InvalidRequest", func(t *testing.T) {
		_, err := k.ActionOfSchemaAll(ctx, nil)
