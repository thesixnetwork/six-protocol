...
func TestActiveEnableVirtualSchemaProposalQuerySingle(t *testing.T) {
	keeper, ctx := keepertest.NftmngrKeeper(t)
	wctx := sdk.WrapSDKContext(ctx)
	msgs := createNActiveEnableVirtualSchemaProposal(keeper, ctx, 2)
	for _, tc := range []struct {
		desc     string
		request  *types.QueryGetActiveEnableVirtualSchemaProposalRequest
		response *types.QueryGetActiveEnableVirtualSchemaProposalResponse
		err      error
	}{
		{
			desc: "First",
			...
			response: &types.QueryGetActiveEnableVirtualSchemaProposalResponse{ActiveEnableVirtualSchemaProposal: msgs[0]},
		},
		{
			desc: "Second",
			request: &types.QueryGetActiveEnableVirtualSchemaProposalRequest{
			...
			response: &types.QueryGetActiveEnableVirtualSchemaProposalResponse{ActiveEnableVirtualSchemaProposal: msgs[1]},
		},
		{
			desc: "KeyNotFound",
			request: &types.QueryGetActiveEnableVirtualSchemaProposalRequest{
		...
		t.Run(tc.desc, func(t *testing.T) {
			response, err := keeper.ActiveEnableVirtualSchemaProposal(wctx, tc.request)
			if tc.err != nil {
				require.ErrorIs(t, err, tc.err)
			} else {
				require.NoError(t, err)
...
func TestActiveEnableVirtualSchemaProposalQueryPaginated(t *testing.T) {
	keeper, ctx := keepertest.NftmngrKeeper(t)
	wctx := sdk.WrapSDKContext(ctx)
	msgs := createNActiveEnableVirtualSchemaProposal(keeper, ctx, 5)

	request := func(next []byte, offset, limit uint64, total bool) *types.QueryAllActiveEnableVirtualSchemaProposalRequest {
		return &types.QueryAllActiveEnableVirtualSchemaProposalRequest{
			Pagination: &query.PageRequest{
				Key:        next,
				Offset:     offset,
				Limit:      limit,
	...
	t.Run("ByOffset", func(t *testing.T) {
		step := 2
		for i := 0; i < len(msgs); i += step {
			resp, err := keeper.ActiveEnableVirtualSchemaProposalAll(wctx, request(nil, uint64(i), uint64(step), false))
			require.NoError(t, err)
	...
	t.Run("ByKey", func(t *testing.T) {
		...
		var next []byte
		for i := 0; i < len(msgs); i += step {
			resp, err := keeper.ActiveEnableVirtualSchemaProposalAll(wctx, request(next, 0, uint64(step), false))
			require.NoError(t, err)
			require.LessOrEqual(t, len(resp.ActiveEnableVirtualSchemaProposal), step)
			require.Subset(t,
				nullify.Fill(msgs),
	...
	t.Run("Total", func(t *testing.T) {
		resp, err := keeper.ActiveEnableVirtualSchemaProposalAll(wctx, request(nil, 0, 0, true))
		require.NoError(t, err)
		require.Equal(t, len(msgs), int(resp.Pagination.Total))
		require.ElementsMatch(t,
			nullify.Fill(msgs),
	...
	t.Run("InvalidRequest", func(t *testing.T) {
		_, err := keeper.ActiveEnableVirtualSchemaProposalAll(wctx, nil)
		require.ErrorIs(t, err, status.Error(codes.InvalidArgument, "invalid request"))
	})
}
