...
func networkWithActionRequestObjects(t *testing.T, n int) (*network.Network, []types.ActionOracleRequest) {
	...
	for i := 0; i < n; i++ {
		...
		state.ActionRequestList = append(state.ActionRequestList, actionRequest)
	}
	buf, err := cfg.Codec.MarshalJSON(&state)
	require.NoError(t, err)
	cfg.GenesisState[types.ModuleName] = buf
	return network.New(t, cfg), state.ActionRequestList
}

func TestShowActionRequest(t *testing.T) {
	net, objs := networkWithActionRequestObjects(t, 2)
	...
	for _, tc := range []struct {
		...
		t.Run(tc.desc, func(t *testing.T) {
			...
			out, err := clitestutil.ExecTestCLICmd(ctx, cli.CmdShowActionRequest(), args)
			if tc.err != nil {
...
func TestListActionRequest(t *testing.T) {
	net, objs := networkWithActionRequestObjects(t, 5)
	...
	t.Run("ByOffset", func(t *testing.T) {
		...
		for i := 0; i < len(objs); i += step {
			...
			out, err := clitestutil.ExecTestCLICmd(ctx, cli.CmdListActionRequest(), args)
			require.NoError(t, err)
