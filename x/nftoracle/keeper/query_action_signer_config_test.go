...
func TestActionSignerConfigQuerySingle(t *testing.T) {
	...
	msgs := createNActionSignerConfig(k, ctx, 2)
	tests := []struct {
		desc     string
	...
	for _, tc := range tests {
		t.Run(tc.desc, func(t *testing.T) {
			response, err := k.ActionSignerConfig(ctx, tc.request)
			if tc.err != nil {
...
func TestActionSignerConfigQueryPaginated(t *testing.T) {
	k, ctx := keepertest.NftoracleKeeper(t)
	msgs := createNActionSignerConfig(k, ctx, 5)
	...
	t.Run("ByOffset", func(t *testing.T) {
		...
		for i := 0; i < len(msgs); i += step {
			resp, err := k.ActionSignerConfigAll(ctx, request(nil, uint64(i), uint64(step), false))
			require.NoError(t, err)
	...
	t.Run("ByKey", func(t *testing.T) {
		...
		var next []byte
		for i := 0; i < len(msgs); i += step {
			resp, err := k.ActionSignerConfigAll(ctx, request(next, 0, uint64(step), false))
			require.NoError(t, err)
			require.LessOrEqual(t, len(resp.ActionSignerConfig), step)
			require.Subset(t,
				nullify.Fill(msgs),
	...
	t.Run("Total", func(t *testing.T) {
		resp, err := k.ActionSignerConfigAll(ctx, request(nil, 0, 0, true))
		require.NoError(t, err)
	...
	t.Run("InvalidRequest", func(t *testing.T) {
		_, err := k.ActionSignerConfigAll(ctx, nil)
