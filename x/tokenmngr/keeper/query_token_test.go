...
func TestTokenQuerySingle(t *testing.T) {
	...
	tests := []struct {
		...
		{
			desc:     "First",
			request:  &types.QueryGetTokenRequest{Name: msgs[0].Name},
			response: &types.QueryGetTokenResponse{Token: msgs[0]},
		},
		{
			desc:     "Second",
			request:  &types.QueryGetTokenRequest{Name: msgs[1].Name},
			response: &types.QueryGetTokenResponse{Token: msgs[1]},
		},
		{
			desc:    "KeyNotFound",
			request: &types.QueryGetTokenRequest{Name: strconv.Itoa(100000)},
